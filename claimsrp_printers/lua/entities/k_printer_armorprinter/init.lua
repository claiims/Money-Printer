...
local function SetValues( ent )
	ent.printTime = 9; -- Default print time.
	...
	ent.printerColor = Color( 66, 134, 244 ); -- The color of the printer prop.
...
function ENT:Use( activator )
	...
	if ( activator:IsPlayer() and self:GetNWInt( "Amount" ) > 0 ) and activator:Armor() < 100 then

		local toGive = math.min(self:GetNWInt("Amount",0), 100 - activator:Armor())

		activator:SetArmor(activator:Armor() + toGive)
		DarkRP.notify( activator, 0, 4, "You have collected " .. toGive .. " Armor from a " .. self.PrintName );
		self:SetNWInt( "Amount", self:GetNWInt( "Amount", 0 ) - toGive );

	elseif self:GetNWInt( "Amount" ) <= 0 then
		DarkRP.notify( activator, 0, 4, "There is no armor to pick up." )
	elseif activator:Armor() >= 100 then	
		DarkRP.notify( activator, 0, 4, "You already have full armor." );
	end;
