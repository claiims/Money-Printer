...
local MaxMultiplier = 2.0 -- What is the max amount that the multiplier can be. (Float/Decimal value)
...
local function SetValues( ent )
	...
	ent.minPrint = 6; -- Minimum print amount.
	ent.maxPrint = 7; -- Maximum print amount.
	ent.upgradedExtra = ent.maxPrint * 0.5; -- The additional income received on upgraded printers.
	ent.printerColor = Color( 255, 0, 255, 204 ); -- The color of the printer prop.
...
function ENT:CreateMoney()
	...
	if self:GetNWInt("Coolant",0) <= 0 && self:GetNWBool( "CoolantToggle" ) then
