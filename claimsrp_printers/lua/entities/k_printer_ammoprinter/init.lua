...
local function SetValues( ent )
	ent.printTime = 4; -- Default print time.
	ent.minPrint = 1; -- Minimum print amount.
	ent.maxPrint = 1; -- Maximum print amount.
	ent.upgradedExtra = ent.maxPrint * 0.5; -- The additional income received on upgraded printers.
	ent.printerColor = Color( 120, 120, 120 ); -- The color of the printer prop.
	ent.coolantSystem = true; -- Toggles the coolant system.
	ent.coolantLoss = 0.05; -- The Percentage loss for each print of the coolant is enabled.
end;
------------------------------
...
function ENT:Use( activator )
	...
	if ( activator:IsPlayer() and self:GetNWInt( "Amount" ) > 0 ) then
		...
		if activator:GetActiveWeapon() && activator:GetActiveWeapon():IsValid() && string.Left(activator:GetActiveWeapon():GetClass(), 3) == "swb_" && !table.HasValue(NotAllowAmmoPickup,activator:GetActiveWeapon():GetClass()) then

			activator:GiveAmmo(self:GetNWInt( "Amount", 0 ),activator:GetActiveWeapon():GetPrimaryAmmoType())
			DarkRP.notify( activator, 0, 4, "You have collected " .. self:GetNWInt( "Amount" ) .. " bullets to your " .. activator:GetActiveWeapon():GetPrintName() );
			self:SetNWInt( "Amount", 0 );

		else
			DarkRP.notify( activator, 0, 4, "You cannot pick up ammo to this weapon." );
		end
	elseif self:GetNWInt( "Amount" ) <= 0 then
		DarkRP.notify( activator, 0, 4, "There is no ammo to pick up." )
	end;
